...
?></div>
	...
	<input name="up" id="up" type="button" value="Upload Photos" style="background-color: #3b5998; float:right;color:white;" />
        <br/>
<br/>
...
$result =  mysqli_query($con,"SELECT * FROM photos WHERE uploadedby='".$_SESSION['SESS_MEMBER_ID'] ."'  ORDER BY photo_id DESC");
...
while($row=mysqli_fetch_array($result,MYSQLI_ASSOC))
 ...
 echo   '<a href=photocomment.php?id=' . $row["photo_id"] . '>' . "<img width=187 height=170 style='padding:10px;' alt='Unable to View' src='". $row["location"] . "'>" . '</a>';
...
?>
	...
	<div class="disabledBackground" id="pictureDiv1">
	
    	</div>
	<img src="images/cross.png" id="exit3" style="display:none;position:fixed;top:75px;left:990px;z-index:1100;">

	<div id="pictureDiv2" class="profLargeimg"><div class="uploadPanel"><span style="color:black;font-size:18px;font-weight:bold;">
	Upload Photo</span></div>
	...
	<form action="uploadPic.php" method="post" enctype="multipart/form-data">
				...
				<br/>+ Upload Profile Pic	<br/>
  					...
  					<input type="file" id="image1" name="image1" style="width:80px;background-color:white;"> 
