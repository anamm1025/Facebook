...
$_SESSION['pageName']=$_GET['pagename'];
	$_SESSION['pageCategory']=$_GET['category'];
$_SESSION['pageDP']=$_GET['dp'];
	...
	<div class="uploadPhotoButton">
				<br/>+ Upload Profile Pic	<br/>
  					...
  					<input type="file" id="image" name="image" style="width:80px;background-color:white;"> 
  						 ...
  						 <input name="pname" type="hidden" id="name" value="<?php 
  		 echo $_SESSION['pageName'];?>"/>
		 <input name="pposter" type="hidden" id="name" value="<?php 
  		 echo $_SESSION['email'];?>"/>
		...
		<div id="photoDiv1" style="background-color:white;width:495px;height:140px;display:none;z-index:1;padding-left:20px;">
				...
				<form action="uploadStatusPicPage.php" method="post" enctype="multipart/form-data">
        ...
        <form  name="form1" method="post" action="save13.php">
		 ...
		 <input name="name1" type="hidden" id="name" value="<?php 
   		echo $_SESSION['pageDP'];?>"/>
	 
            <input id="postx" type="submit" name="btnlog" style="padding-top:0px;color:white;background-color:darkblue;position:absolute;top:613px;left:1010px;"  value="Post" />
...
$query  = "SELECT *,UNIX_TIMESTAMP() - date_created AS TimeSpent FROM message WHERE 
 email='".$_SESSION['email'] ."' and pageName='".$_SESSION['pageName']."' ORDER BY messages_id DESC";
...
while($row = mysqli_fetch_assoc($result))
{
	...
	echo "<span class='blueName'>" . "{$row['pageName']}</span>";

echo '<span class="delete">';
	echo '<font color="White">';
	echo '<a style="float:right;" class="commentDescription"  href=pdeletepostTimeline.php?id=' . $row["messages_id"] . '>' . "Delete" . '</a>';
	echo '</font>';	
	echo '&nbsp;';
	echo'</span>';
...
if($row['postedpic']!="")
{
	...
	echo '<form action="plike.php" method="post">'; 
...
echo '<a style="float:right;right-margin:200px;font-size:small;color:navy;" href=pdeletepostcommentTimeline.php?id=' . $row1["comment_id"] . '>' . "DELETE" . '</a>';
...
echo '<hr></br>';
	...
	echo '<form action="pcommentpost.php" method="post">'; 
	echo'<input type="hidden" name="postid" value="'. $row['messages_id'] .'">';
	echo'<input type="hidden" name="user" value="'. $_SESSION['pageName'] .'">';
echo'<input type="hidden" name="email" value="'. $_SESSION['email'] .'">';
	echo'<input type="hidden" name="pic" value="'. $_SESSION['pageDP'] .'">';
